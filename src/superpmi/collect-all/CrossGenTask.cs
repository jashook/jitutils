...
public class CrossGenTask : ISuperPMITask
{
   ...
   public CrossGenTask(string settingsFile = null)
   {
      ...
      string defaultMchLocation = Path.Combine(instance.m_testRoot, name + ".mch");
      ...
      this.m_command += $"-run superpmicrossgen crossgen -v -c {crossGenPath} --shimJit {shimJitPath} {assemblyArgs}";
   }
   ...
   public void RunTask()
   {
      ...
      proc.StartInfo.FileName = "SuperPMICollect";
      proc.StartInfo.Arguments = this.m_command;

      if (Utilities.GetInstance().m_loggingVerbose)
      {
         Console.WriteLine("SuperPMICollect" + " " + this.m_command);
         Console.WriteLine(" ");
      }
